...
def read_sir_csv(path_to_input_network, columns=None, ADelimiter=DEFAULT_DELIMITER):
    return np.loadtxt(path_to_input_network, dtype=str, delimiter=ADelimiter, usecols=columns)
...
def infection_gradient(current_infections, total_population):
    percent_infected = current_infections / total_population
...
def tuple_of_compartments_cols(number_of_compartments):
    cols = []
    for i in range(1, number_of_compartments+1):
        cols.append(i)
